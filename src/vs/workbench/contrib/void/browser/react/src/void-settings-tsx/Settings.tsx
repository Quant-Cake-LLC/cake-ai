...
export const FeaturesTab = () => {
	return <>
		...
		{/* <h3 className={`mb-2`}>{`CakeAI can access any model that you host locally. We automatically detect your local models by default.`}</h3> */}
		<h3 className={`text-void-fg-3 mb-2`}>{`CakeAI can access any model that you host locally. We automatically detect your local models by default.`}</h3>
		<div className='pl-4 opacity-50'>
			...
			<span className={`text-sm mb-2`}><ChatMarkdownRender noSpace string={`CakeAI automatically detects locally running models and enables them.`} /></span>
		...
		<h3 className={`text-void-fg-3 mb-2`}>{`CakeAI can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>
...
const transferTheseFilesOfOS = (os: 'mac' | 'windows' | 'linux' | null): TransferFilesInfo => {
	...
	if (os === 'mac') {
		...
		return [{
			...
			to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'CakeAI', 'User', 'settings.json'),
		}, {
			from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Code', 'User', 'keybindings.json'),
			to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'CakeAI', 'User', 'keybindings.json'),
		}, {
	...
	if (os === 'linux') {
		...
		return [{
			...
			to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'CakeAI', 'User', 'settings.json'),
		}, {
			from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Code', 'User', 'keybindings.json'),
			to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'CakeAI', 'User', 'keybindings.json'),
		}, {
			from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.vscode', 'extensions'),
			to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.void-editor', 'extensions'),
		}]
	...
	if (os === 'windows') {
		...
		return [{
			...
			to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'CakeAI', 'User', 'settings.json'),
		}, {
			from: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Code', 'User', 'keybindings.json'),
			to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'CakeAI', 'User', 'keybindings.json'),
		}, {
...
const GeneralTab = () => {
	...
	return <>
		...
		<div className=''>
			...
			<h4 className={`text-void-fg-3 mb-2`}>{`Transfer your settings from VS Code to CakeAI in one click.`}</h4>
	...
	return <div className={`@@void-scope ${isDark ? 'dark' : ''}`} style={{ height: '100%', width: '100%' }}>
		<div ref={containerRef} className='overflow-y-auto w-full h-full px-10 py-10 select-none'>
			...
			<div className='max-w-5xl mx-auto'>
				...
				<h1 className='text-2xl w-full'>CakeAI Settings</h1>

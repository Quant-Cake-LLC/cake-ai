...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	override readonly minimumWidth: number = 230; // CakeAI changed this (was 170)
	...
	private fillExtraContextMenuActions(actions: IAction[]): void {
		...
		actions.push(...[
			...
			toAction({ id: ToggleSidebarPositionAction.ID, label: currentPositionRight ? localize('move second side bar left', "Move CakeAI Side Bar Left") : localize('move second side bar right', "Move CakeAI Side Bar Right"), run: () => this.commandService.executeCommand(ToggleSidebarPositionAction.ID) }),
			toAction({ id: ToggleAuxiliaryBarAction.ID, label: localize('hide second side bar', "Hide CakeAI Side Bar"), run: () => this.commandService.executeCommand(ToggleAuxiliaryBarAction.ID) })
		]);
	}
